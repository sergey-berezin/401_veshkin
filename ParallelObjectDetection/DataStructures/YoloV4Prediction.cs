...
namespace ParallelObjectDetection.DataStructures
{
    public class YoloV4Prediction
    {
        ...
        public IReadOnlyList<YoloV4Result> GetResults(BufferBlock<KeyValuePair<string, YoloV4Result>> resultBuffer,
                                                      string imagePath,
                                                      string[] categories,
                                                      ...
                                                      float iouThres = 0.5f)
        {
            ...
            while (f < postProcesssedResults.Count)
            {
                ...
                var foundResult = new YoloV4Result(res.Take(4).ToArray(), label, conf);
                resultBuffer.Post(new KeyValuePair<string, YoloV4Result>(imagePath, foundResult));
                resultsNms.Add(foundResult);
        ...
        public IReadOnlyList<YoloV4Result> GetResults(string imagePath,
                                                      string[] categories,
                                                      float scoreThres = 0.5f,
                                                      float iouThres = 0.5f)
        {
